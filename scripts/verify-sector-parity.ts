...
async function loginViaHttp(): Promise<string | null> {
  ...
  const { csrfToken } = (await csrfRes.json()) as {
    csrfToken: string;
  };
  ...
  const cookies = [
    ...csrfCookies,
    ...collectSetCookies(loginRes.headers),
  ];
...
async function main() {
  ...
  const packageTickets = ticketsFromUmrahApiItems(
    items,
    markupPercent
  );
  ...
  const groupFlights = cookie
    ? await fetchTravelLineGroupFlights(cookie)
    : [];

  const groupTickets = ticketsFromGroupFlights(
    groupFlights,
    markupPercent
  );
  ...
  const wrongDirection = merged.filter(
    (t) => !isOutboundGroupTicket(t.from, t.to)
  );
  ...
  for (const item of items) {
    ...
    const resolvedFrom = resolveAirport(
      item.departureSectorFrom
    );

    const resolvedTo = resolveAirport(
      item.departureSectorTo
    );

    // If the supplier sector cannot be resolved,
    // skip this item rather than causing the verification
    // script to fail with a null reference.
    if (!resolvedFrom || !resolvedTo) {
      continue;
    }

    const expectedFrom = resolvedFrom.code;
    const expectedTo = resolvedTo.code;

    const ticket = packageTickets.find(
      (t) => t.externalId === `${item.id}-out`
    );
    ...
    if (
      ticket.from !== expectedFrom ||
      ticket.to !== expectedTo
    ) {
  ...
  const report = {
    ...
    wrongDirection: wrongDirection.map(
      (t) =>
        `${t.flightNumber} ${t.from}→${t.to} (${t.date}) id=${t.externalId}`
    ),
