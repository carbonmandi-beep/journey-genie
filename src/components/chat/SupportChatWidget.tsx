...
const STARTER: Msg = {
  role: "assistant",
  content:
    "Hi! I'm Journey Genie's travel assistant. Ask me about flights, hotels, holiday packages, visa assistance, or travel planning. Pick a suggestion below or type your question.",
};

const SUGGESTIONS = [
  "Find flights from Delhi to Dubai",
  "What holiday packages do you offer?",
  "Help me find a hotel",
  "What visa assistance do you provide?",
  "How can I book through WhatsApp?",
] as const;
...
export function SupportChatWidget() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "journeygenie:open-support-chat",
      openChat
    );
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          json.error ||
            "Could not reach Journey Genie support."
        );
      ...
      setError(
        "Network error. Please try again or continue on WhatsApp."
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      {open && (
        <div className="flex h-[min(74vh,620px)] w-[min(100vw-1.25rem,400px)] flex-col overflow-hidden rounded-[1.35rem] border border-white/20 bg-[#0b1f3a] shadow-[0_28px_90px_rgba(4,18,40,.45)]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-white/55">
                    Online · flights, hotels & travel help
                  </p>
                </div>
              </div>
          ...
          <div
            ref={listRef}
            className="flex-1 space-y-3 overflow-y-auto bg-[#f3f0ea] px-3.5 py-4"
          >
            {messages.map((m, i) => (
              <div
                key={`${m.role}-${i}`}
      ...
      <button
        ...
        aria-label={
          open
            ? "Close Journey Genie Assistant"
            : "Open Journey Genie Assistant"
        }
      >
